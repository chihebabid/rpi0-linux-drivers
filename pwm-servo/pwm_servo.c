...
static ssize_t mypwm_write(struct file *flip,const char *buf,size_t len,loff_t *off) {
	pr_info("mydev_write() is called.\n");
		...
		if (kstrtoint(kbuf, 10, &degree)) {
			pr_err("Failed to parse input\n");
			return -EINVAL;
		}

		if (degree < 0 || degree > 180) {
			pr_err("Degree must be between 0 and 180\n");
			return -EINVAL;
		}
		mydev->angle_degree=degree;
		u64 duty_cycle_ns=((MAX_DUTY_NS-MIN_DUTY_NS)*degree/180)+MIN_DUTY_NS; // Compute duty cycle
...
static int pwm_platform_probe(struct platform_device *pdev)
{
	...
	mydev->angle_degree=0;
	mydev->current_duty_ns=MIN_DUTY_NS;
